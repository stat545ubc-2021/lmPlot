...
1. Example that works: Plot the relationship between `cyl` - Number of cylinders and `mpg` - Miles/(US) gallon of the `mtcars` data.

```{r, error = TRUE}
plot_lm(mtcars, cyl, mpg, na.rm = T)
```

2. Example of errors: Choose the `continent` column from the `gapminder` dataset as the `x` variable. It's not `numeric` so can't fit in the linear model.
```{r, error = TRUE}
